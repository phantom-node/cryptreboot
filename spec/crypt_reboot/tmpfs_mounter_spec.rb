...
module CryptReboot
  RSpec.describe TmpfsMounter do
    ...
    it 'raises error if mount failed' do
      mounter = described_class.new(mounter: ->(_dir) { raise ZeroDivisionError }, exception_class: ZeroDivisionError)
      expect do
        mounter.call('d1') { raise StandardError }
      end.to raise_error(TmpfsMounter::Error)
    end
